...
<body class="bg-gray-50 min-h-screen flex">
        ...
        <div class="px-4 py-4 border-t border-white/10">
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('logout') }}" class="ml-auto">
                    @csrf
                    <button type="submit" class="text-white/30 hover:text-white/70 transition text-sm">x</button>
                </form>
            </div>
        </div>
    </aside>
    ...
    <main class="ml-52 flex-1 p-6">
        ...
        {{-- Carian & Filter --}}
        <form method="GET" action="{{ route('admin.peranti') }}" class="flex gap-3 mb-5">
            <input
                type="text"
                name="cari"
                value="{{ request('cari') }}"
            ...
            <select name="status" class="bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-200">
                <option value="">Semua Status</option>
                <option value="1" {{ request('status') === '1' ? 'selected' : '' }}>Aktif</option>
                <option value="0" {{ request('status') === '0' ? 'selected' : '' }}>Tidak Aktif</option>
            </select>
            <button type="submit" class="bg-gray-800 text-white text-sm font-semibold px-5 py-2.5 rounded-xl hover:bg-gray-700 transition">
                Cari
            </button>
        </form>
        ...
        <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            <div class="px-5 py-3.5 border-t border-gray-100 flex items-center justify-between">
                <p class="text-xs text-gray-400">
                    Menunjukkan {{ $peranti->firstItem() ?? 0 }} - {{ $peranti->lastItem() ?? 0 }}
                    daripada {{ $peranti->total() ?? 0 }} rekod
                </p>
