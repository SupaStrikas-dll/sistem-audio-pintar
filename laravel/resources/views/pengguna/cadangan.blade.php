    ...
    <main class="ml-56 flex-1 p-6">
        ...
        <div class="grid grid-cols-3 gap-5">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden card-hover">
                ...
                {{-- Gambar Peranti --}}
                @if($item->peranti->imej)
                <img src="{{ asset($item->peranti->imej) }}"
                    alt="{{ $item->peranti->nama }}"
                ...
                <div class="h-36 flex items-center justify-center text-5xl"
                    ...
                    @if(($item->peranti->kategori->nama_kategori ?? '') === 'Speaker') 🔊
                    @elseif(($item->peranti->kategori->nama_kategori ?? '') === 'Mikrofon') 🎤
                    @elseif(($item->peranti->kategori->nama_kategori ?? '') === 'Earphone') 🎵
                    @else 🎧
                    @endif
                </div>
                @endif
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @if($loop->first)
                        <span class="text-xs font-bold bg-yellow-50 text-yellow-600 px-2.5 py-1 rounded-full">
                            Terbaik
                        </span>
                        @endif
                    </div>
                    ...
                    <p class="text-xs text-gray-400 mb-2">
                        {{ $item->peranti->jenama }} • {{ $item->peranti->kategori->nama_kategori ?? '-' }}
                    </p>

                    {{-- Harga --}}
                    <p class="text-base font-bold text-blue-600 mb-3">
                        RM {{ number_format($item->peranti->harga, 2) }}
                    </p>

                    {{-- Tag Kategori --}}
                    <div class="flex flex-wrap gap-1.5 mb-3">
                        <span class="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                            {{ $item->peranti->kategori->nama_kategori ?? '-' }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-1 mb-3">
                        @for($i = 1; $i <= 5; $i++)
                            <span class="text-xs {{ $i <= round($item->peranti->skor_purata ?? 0) ? 'text-yellow-400' : 'text-gray-200' }}">★</span>
                            @endfor
                            <span class="text-xs text-gray-400">{{ number_format($item->peranti->skor_purata ?? 0, 1) }}</span>
                    </div>

                    {{-- Butang Detail --}}
                    <a href="{{ route('peranti.detail', $item->peranti->id) }}"
